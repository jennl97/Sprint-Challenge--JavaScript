...
*/
const animalNames = [];
zooAnimals.forEach((type) => {
  animalNames.push(`${type.animal_name}, ${type.scientific_name}`);
});
...
let lowerCase = [];
lowerCase = zooAnimals.map((type) => {
  return type.animal_name.toLowerCase();
})
...
*/
let lowerPopulation = [];
lowerPopulation = zooAnimals.filter((type) => {
  return type.population < 5;
})
